		...
		<div class="card my-1">
			...
			<div id="collapseOne" class="collapse">
				<div class="card-body">
					...
					<p>这个时候UOJ出现了，于是<?= UOJConfig::$data['profile']['oj-name-short'] ?>就基于UOJ这套系统进行了一系列修改。Universal的中文意思是通用，之所以称之为UOJ，因为我们所有题目从编译、运行到评分，都可以由出题人自定义。</p>
					...
					<p>所以我们引入了Extra Tests和Hack机制。每道传统题的数据都分为Tests和Extra Tests，Tests满分90分，如果你通过了所有的Tests，那么就会为你测Extra Tests。如果过了Tests但没过Extra Tests那么倒扣3分变为87分。Extra Tests的来源，一个是这道题没什么人可能会错的边界情况可以放在里面，另一个就是各位平时做题的时候，如果发现错误算法AC了，可以使用hack将其卡掉，<?= UOJConfig::$data['profile']['oj-name-short'] ?>会自动加入Extra Tests并重测。我们无法阻止暴力高分的脚步，但是不让他得满分还是有心里安慰作用的～</p>
					<p>平时做比赛时我很难容忍的一个地方就是平时做模板题时常数就很大，导致比赛时被卡常。</p>
					<p>所以我们引入了动态时限机制，你的用时占10分。<b>每道题只有跑入最优解的两倍时间以内才能获得满分</b>，否则将会获得90分到99分不等的分数（不过我们也会给被卡常的代码AC的反馈的~）。选手从平时做题就学会了控制常数，比赛时被卡常的概率也会降低！</p>
